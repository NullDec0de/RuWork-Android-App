...
public class ActivityBookingRequestDetail extends BaseActivity implements TabLayout.OnTabSelectedListener, RetrofitHandler.RetrofitResHandler {
    ...
    @BindView(R.id.rl_parentcod)
    RelativeLayout rl_parentCod;

    @BindView(R.id.tv_mark_completed_cod)
    TextView tv_mark_completed_cod;
    @BindView(R.id.iv_mark_acceptcod)
    ImageView iv_mark_acceptcod;
            ...
            case AppConstants.BOOKINGDETAIL:
                ...
                if (PreferenceStorage.getKey(AppConstants.USER_TYPE).equalsIgnoreCase("1")) {
                    if (daoBookingDetail.getData().getBookingDetails().getStatus().equalsIgnoreCase("1")) {
                        ...
                        if (daoBookingDetail.getData().getBookingDetails().getCod().equalsIgnoreCase("1")){
                            rl_parentCod.setVisibility(View.VISIBLE);
                        } else {
                            rl_parentCod.setVisibility(View.GONE);
                        }
                    ...
                    } else if(daoBookingDetail.getData().getBookingDetails().getStatus().equalsIgnoreCase("8")){
                        btnRatenow.setVisibility(View.VISIBLE);
                    }
                ...
                adapter.addFragment(new ServiceAboutBuyerFragment(daoBookingDetail.getData()
                        .getUserDetails(), daoBookingDetail.getData().getBookingDetails()),
                        AppUtils.cleanLangStr(this, about, R.string.txt_request));
    ...
    @OnClick({R.id.iv_reject, R.id.btn_accept, R.id.btn_reject, R.id.iv_mark_accept, R.id.iv_completed_cod, R.id.iv_complete_accept, R.id.iv_complete_reject, R.id.btn_ratenow, R.id.btn_cancel})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_reject:
                updateserviceStatus("5", ""); // provider reject
                break;
            ...
            case R.id.iv_complete_accept:
                ...
                break;
            case R.id.iv_completed_cod:
                updateserviceStatus("8", ""); // User complete accept
                break;
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (etComments.getText().toString().isEmpty()) {
                    ...
                    updateserviceStatus("2", etComments.getText().toString().trim());
                    dialog.dismiss();
                }
            }
        });
