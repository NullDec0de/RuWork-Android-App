...
public class CardListActivity extends AppCompatActivity implements View.OnClickListener, RetrofitHandler.RetrofitResHandler,
    ...
    RadioButton radioButtonDebitCreditCard, rb_stripe_Credit_cards;
    RadioButton paypal, paystack, paySolutions;
    ...
    String paymentType = "", customeremail="", merchantid="";
...
//        ivUserlogin.setVisibility(View.GONE);
        ...
        rb_stripe_Credit_cards = findViewById(R.id.rb_stripe_credit_cards);
        ...
        currentTimetxt = Calendar.getInstance().getTime();
...
//        rvSavedCards.setHasFixedSize(true);
        ...
        Intent intent = new Intent(CardListActivity.this, PayStackActivity.class);
        intent.putExtra(AppConstants.CASH_AMOUNT, newcurrency);
        intent.putExtra(AppConstants.ORDERID, currentTimetxt.toString());
...
//                    walletScreenList.getLblWithdrawWallet().getName(), R.string.txt_withdraw_wallet));
            ...
            radioGroupAddNewCards.setVisibility(View.VISIBLE);
            ...
            tvGigsTitle.setText(AppUtils.cleanLangStr(this,
                    walletScreenList.getLblWithdrawWallet().getName(), R.string.txt_withdraw_wallet));
            rb_stripe_Credit_cards.setVisibility(View.GONE);
//            setToolBarTitle(AppUtils.cleanLangStr(this,
        ...
        radioGroupAddNewCards.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                if (checkedId == radioButtonDebitCreditCard.getId()) {
                    ...
                    paymentType = "3";
                    linearLayoutAddNewCardPanel.setVisibility(View.GONE);
                }else if (checkedId == paystack.getId()) {
                    paymentType = "4";
                    linearLayoutAddNewCardPanel.setVisibility(View.GONE);
                } else if(checkedId == rb_stripe_Credit_cards.getId()){
                    paymentType = "5";
                }else if(checkedId == paySolutions.getId()){
                    paymentType = "6";
                }
            }
        });
...
//            rb_stripe_Credit_cards.setText(AppUtils.cleanLangStr(this,
//                    walletScreenList.getLblDebitCreditCard().getName(), R.string.text_debit_card_credit_card));
                ...
                if (fromPage.equalsIgnoreCase(AppConstants.WITHDRAW)) {
                    ...
                    if(Double.parseDouble(walletAmount) == 0){
                        Toast.makeText(this, AppUtils.cleanLangStr(this,
                                "Wallet Amount is Empty", R.string.txt_wallet_amount_empty), Toast.LENGTH_SHORT).show();
                        return;
                    } else {
                        if(accountSaved==null) {
                            ...
                            return;
                        } else  if (accountSaved.equalsIgnoreCase("0")) {
                            startActivityForResult(new Intent(this, StripeSettingsActivity.class)
                                            .putExtra("fromPage", "withdraw"),
                                    SAVE_ACCOUNT_REQUEST_CODE);
                            return;
...
//                        onBrainTreeSubmit();
                        ...
                        currencyconversion();
                          ...
                          }else if (paySolutions.isChecked()) {
                        if (editTextAmount.getText().toString().isEmpty()) {
                            Toast.makeText(this, AppUtils.cleanLangStr(this,
                                    walletScreenList.getLblEnterAmtToProceed().getName(), R.string.txt_enter_amount), Toast.LENGTH_SHORT).show();
                            return;
            ...
            apiInterface.postCurrencyconversion(data, PreferenceStorage.getKey(AppConstants.USER_TOKEN)).enqueue(new Callback<POSTCurrencyConversion>() {
                ...
                public void onResponse(Call<POSTCurrencyConversion> call, Response<POSTCurrencyConversion> response) {
                    ...
                    if (response.body().getResponse().getResponseCode().equals("200")) {
                        ...
                        Toast.makeText(CardListActivity.this, response.body().getResponse().getResponseMessage(), Toast.LENGTH_LONG).show();
//                        Utils.toastMessage(PayStackActivity.this, response.body().getMessage());
                ...
                public void onFailure(Call<POSTCurrencyConversion> call, Throwable t) {
                    ...
                    ProgressDlg.dismissProgressDialog();
                    finish();
                    Toast.makeText(CardListActivity.this, "Paystack not working}", Toast.LENGTH_LONG).show();
                }
            });
        } else {
            ProgressDlg.dismissProgressDialog();
            Toast.makeText(CardListActivity.this, "Enable Internet", Toast.LENGTH_LONG).show();
        }
    ...
    private void updateUI() {
        ...
        intent.putExtra(AppConstants.ORIGINAL_AMOUNT, editTextAmount.getText().toString());
        intent.putExtra(AppConstants.ORDERID, currentTimetxt.toString());
        intent.putExtra(AppConstants.FROM, "MYACTIVITY");
        ...
        if (AppUtils.isNetworkAvailable(this)) {
            ...
            if(paymentType.equals("")){
                ...
                Call<BaseResponse> classificationCall = apiService.walletWithdraw(PreferenceStorage.getKey(AppConstants.USER_TOKEN),
                        editTextAmount.getText().toString(),"stripe", currencycode);
                ...
                ApiInterface apiService =
                        ApiClient.getClientNoHeader().create(ApiInterface.class);
                Call<BaseResponse> classificationCall = apiService.walletWithdrawPaypl(PreferenceStorage.getKey(AppConstants.USER_TOKEN),
                        editTextAmount.getText().toString(), PreferenceStorage.getKey(AppConstants.USER_TOKEN) ,"paypall", currencycode);
                RetrofitHandler.executeRetrofit(this, classificationCall, AppConstants.WITHDRAWWALLET, this, false);
            }else if(paymentType.equals("3")){
                ApiInterface apiService =
                        ApiClient.getClientNoHeader().create(ApiInterface.class);
                Call<BaseResponse> classificationCall = apiService.walletWithdraw(PreferenceStorage.getKey(AppConstants.USER_TOKEN),
                        editTextAmount.getText().toString(),"razorpay", currencycode);
                RetrofitHandler.executeRetrofit(this, classificationCall, AppConstants.WITHDRAWWALLET, this, false);
            }else if(paymentType.equals("4")){
                ApiInterface apiService =
                        ApiClient.getClientNoHeader().create(ApiInterface.class);
                Call<BaseResponse> classificationCall = apiService.walletWithdraw(PreferenceStorage.getKey(AppConstants.USER_TOKEN),
                        editTextAmount.getText().toString(),"paystack", currencycode);
                RetrofitHandler.executeRetrofit(this, classificationCall, AppConstants.WITHDRAWWALLET, this, false);
            }
    ...
    public void onSuccess(Object myRes, boolean isLoadMore, String responseType) {
        ...
        switch (responseType) {
            ...
            case AppConstants.STRIPEDETAILS:
                ...
                publicKey = stripeDetailsModel.getData().getPublishableKey();
                ...
                paystackref = String.valueOf(stripeDetailsModel.getData().getPaystackRefKey());


                PreferenceStorage.setKey(AppConstants.USER_EMAIL, customeremail);

                PreferenceStorage.setKey(AppConstants.MERCHANTID, merchantid);

                PreferenceStorage.setKey(AppConstants.PAYSTACKREF, paystackref);
...
//        super.onResponseFailure(myRes, isLoadMore, responseType);
    ...
    public void startPaysolutions() {
        ...
        String nuuu = String.valueOf(number);

        Intent i = new Intent(CardListActivity.this, WebViewActivity.class);
        i.putExtra(AppConstants.TbTitle, AppUtils.cleanLangStr(CardListActivity.this,
                "Paysolutions", R.string.txt_make_a_suggestion));
//        String urrrl = "https://truelysell.dreamguystech.com/home/paysolutionapi?" +  PreferenceStorage.getKey(AppConstants.PAYSTACKREF) + "&"  + nuuu + "&" + PreferenceStorage.getKey(AppConstants.MERCHANTID) + "&" + PreferenceStorage.getKey(AppConstants.USER_EMAIL) +"&" +"productdetail=wallet" + "&" + editTextAmount.getText().toString();
        String urrrl = "https://truelysell.dreamguystech.com/home/paysolutionapi?" + "refno="+  paystackref + "&" + "merchantid="  + merchantid + "&" + "customeremail=" + customeremail +"&" +"productdetail=wallet" + "&" +"total=" +  editTextAmount.getText().toString();
