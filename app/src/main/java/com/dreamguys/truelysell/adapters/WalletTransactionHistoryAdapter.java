...
public class WalletTransactionHistoryAdapter extends RecyclerView.Adapter<WalletTransactionHistoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull WalletTransactionHistoryAdapter.ViewHolder holder, int position) {
        ...
        try {
            ...
            if (mData.get(position).getCreditWallet().equalsIgnoreCase("0.00")||mData.get(position).getCreditWallet().equalsIgnoreCase("0")) {
            ...
            holder.tvDate.setText(new SimpleDateFormat(PreferenceStorage.getKey(AppConstants.DATEFORMAT)).format(date));

            holder.LLtransaction.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {


                    Intent walletDetail = new Intent(mContext, WalletDetails.class)
                            .putExtra("ID", mData.get(position).getId())
                            .putExtra("currency", mData.get(position).getCurrency());
                    mContext.startActivity(walletDetail);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            LLtransaction = itemView.findViewById(R.id.ll_transaction);
        }
    }
}
