...
public class SelectDateTimeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        if(!PreferenceStorage.getKey(AppConstants.TIMEFORMAT).equals("12hrs")){
            String chngtime1 = parseDateToddMMyyyy(itemsData.get(position).getStartTime());

            String chngtime2 = parseDateToddMMyyyy(itemsData.get(position).getEndTime());

            categoryViewHolder.tvFromtime.setText(chngtime1);
            categoryViewHolder.tvTotime.setText(chngtime2);
        } else {
