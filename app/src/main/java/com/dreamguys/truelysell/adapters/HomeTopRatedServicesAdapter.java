...
public class HomeTopRatedServicesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    TextView tvCategory;
    int appColor;
    List<GETHomeList.TopRatedService> tio_rated_services = new ArrayList<>();
    private HomeFragment homeFragment;

    public HomeTopRatedServicesAdapter(Context mContext, List<GETHomeList.TopRatedService> tio_rated_services,
                                       HomeFragment homeFragment) {
        this.mContext = mContext;
        this.tio_rated_services = tio_rated_services;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent,
                                                      int viewType) {
        View itemView;
        itemView = LayoutInflater.from(mContext).inflate(R.layout.popularservnew, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                callServiceDetailAct.putExtra(AppConstants.SERVICEID, tio_rated_services.get(position).getServiceId());
                callServiceDetailAct.putExtra(AppConstants.SERVICETITLE, tio_rated_services.get(position).getServiceTitle());
                mContext.startActivity(callServiceDetailAct);
            }
        });

        NewViewHolder.rbRating.setRating(Float.parseFloat(tio_rated_services.get(position).getRatings()));
        NewViewHolder.tvRatingCount.setText("(" + tio_rated_services.get(position).getRatingCount() + ")");
        NewViewHolder.tvCategory.setText(tio_rated_services.get(position).getCategoryName());
        NewViewHolder.tvServiceName.setText(tio_rated_services.get(position).getServiceTitle());

//        Location endPoint = new Location("locationB");
//        endPoint.setLatitude(Double.parseDouble(tio_rated_services.get(position).getServiceLatitude()));
//        endPoint.setLongitude(Double.parseDouble(tio_rated_services.get(position).getServiceLongitude()));
//
//        Location startPoint = new Location("locationA");
//        startPoint.setLatitude(Double.parseDouble(PreferenceStorage.getKey(AppConstants.MY_LATITUDE)));
//        startPoint.setLongitude(Double.parseDouble(PreferenceStorage.getKey(AppConstants.MY_LONGITUDE)));
//
//        float distance = startPoint.distanceTo(endPoint);
//
//        String val = String.format("%.0f", distance);
//
//
//        NewViewHolder.Tv_lovtxt.setText(val + " km");

                NewViewHolder.Tv_lovtxt.setText(" km");


        NewViewHolder.tvServicePrice.setText(Html.fromHtml(tio_rated_services.get(position).getCurrency()) + tio_rated_services.get(position).getServiceAmount());

        Glide.with(mContext)
                .load(AppConstants.BASE_URL + tio_rated_services.get(position).getServiceImage())
        ...
        Glide.with(mContext)
                .load(AppConstants.BASE_URL + tio_rated_services.get(position).getUserImage())
                .apply(new RequestOptions().error(R.drawable.usericnew).placeholder(R.drawable.usericnew).transforms(new CenterCrop(), new RoundedCorners(40)))
                .into(NewViewHolder.ivProfileImage);

        if (tio_rated_services.get(position).getService_favorite() != null) {
            if (tio_rated_services.get(position).getService_favorite().equalsIgnoreCase("1")) {
        ...
        NewViewHolder.icFav.setOnLikeListener(new OnLikeListener() {
            ...
            public void liked(LikeButton likeButton) {
                homeFragment.callFav(tio_rated_services.get(position).getServiceId(), "1");
            }

            @Override
            public void unLiked(LikeButton likeButton) {
                homeFragment.callFav(tio_rated_services.get(position).getServiceId(), "0");
            }
        });
    ...
    public int getItemCount() {
        return tio_rated_services.size();
